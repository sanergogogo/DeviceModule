...
@ccclass('CQRCode')
export class CQRCode extends Component {
    @property
    ...
    private _backColor: Color = Color.WHITE;
    private _foreColor: Color = Color.BLACK;
    ...
    public updateContent() {
        ...
        transform.width = this.size;
        transform.height = this.size;
        ...
        g.clear();
        // 背景色
        g.fillColor = this.backColor;
        let width = this.size;
        let offsetX = -width * transform.anchorX;
        let offsetY = -width * transform.anchorY;
        g.rect(offsetX, offsetY, width, width);
        g.fill();
        g.close();
        ...
        for (let row = 0; row < num; row++) {
            for (let col = 0; col < num; col++) {
                if (qrcode.isDark(row, col)) {
                    g.rect(offsetX + this.margin + col * tileW, offsetX + size - tileH - Math.round(row * tileH) + this.margin, w, h);
                    g.fill();
                }
            }
        }
